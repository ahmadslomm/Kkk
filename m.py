...
# ── فلتر الوقت الهادئ ──
QUIET_HOURS_START    = 1    # من الساعة 1 صباحاً
QUIET_HOURS_END      = 5    # حتى الساعة 5 صباحاً (UTC)
QUIET_VOLUME_MULT    = 0.5  # السوق بيكون هادي = نص الحجم الطبيعي

# ── Smart Cooldown ──
COOLDOWN_SCORE_6     = 300   # 5 دقائق لإشارة 6/6
COOLDOWN_SCORE_5     = 600   # 10 دقائق لإشارة 5/6
COOLDOWN_SCORE_4     = 900   # 15 دقائق لإشارة 4/6

# ── Trailing Stop ──
TRAILING_STOP_PCT    = 0.015  # 1.5% trailing stop
TRAILING_CHECK_SEC   = 60     # فحص كل دقيقة

# ── Volume Profile ──
VOLUME_PROFILE_BINS  = 20     # عدد مستويات الـ VP
...
trailing_stops       = {}   # symbol -> {"peak": float, "stop": float, "entry": float}

# ── Price history للـ Volume Profile ──
price_history        = defaultdict(deque)  # symbol -> deque of (price, volume)
...
# ── Performance tracker ──
signal_performance   = defaultdict(list)   # symbol -> list of pct changes

# ── Market microstructure ──
spread_history       = defaultdict(list)   # symbol -> list of spreads

# ── Momentum tracker ──
momentum_scores      = {}   # symbol -> momentum score

# ── Divergence tracker (RSI Divergence) ──
divergence_cache     = {}

# ── Multi-timeframe cache ──
mtf_cache            = {}   # symbol -> {tf: df}
mtf_cache_ts         = {}

# ── Heatmap data ──
sector_performance   = defaultdict(list)   # sector -> list of % changes

# ── Anomaly detection ──
volume_baseline      = defaultdict(list)   # symbol -> list of volumes (baseline)

# ── Liquidity zones ──
liquidity_zones      = defaultdict(list)   # symbol -> list of (price, volume)

# ── Fear & Greed approximation ──
fear_greed_score     = 50   # 0=خوف شديد, 100=جشع شديد
last_fg_update       = 0
...
def calc_stoch_rsi(closes, rsi_period=14, stoch_period=14):
    """Stochastic RSI — أدق من RSI العادي"""
    rsi_values = []
...
def calc_macd(closes, fast=12, slow=26, signal=9):
    ...
    macd_cross   = macd_line[-1] > signal_line[-1] and macd_line[-2] <= signal_line[-2] if len(macd_line) > 1 and len(signal_line) > 1 else False
    return {
...
def calc_bollinger(closes, period=20, std_dev=2):
    """Bollinger Bands — كشف الضغط والانفجار"""
    ...
    squeeze= bw < 2.0   # Bollinger Squeeze = طاقة متراكمة
    return {
...
def calc_volume_profile(df, bins=VOLUME_PROFILE_BINS):
    """Volume Profile — أين يتمركز الحجم"""
    prices  = df["c"].values
    volumes = df["v"].values
    ...
    for p, v in zip(prices, volumes):
        ...
        profile[bucket] += v
    # Point of Control = السعر بأعلى حجم
...
def detect_rsi_divergence(df):
    """Bullish Divergence = سعر ينزل لكن RSI يطلع = انعكاس قريب"""
...
def get_mtf_trend(symbol):
    """تحليل متعدد الإطارات: 1m + 5m + 15m"""
    trends = {}
    for tf in ["1m", "5m", "15m"]:
        try:
...
def detect_support_resistance(df):
    """كشف مستويات الدعم والمقاومة الحقيقية"""
    highs   = df["h"].values
    lows    = df["l"].values
    closes  = df["c"].values
    current = closes[-1]
...
def detect_liquidity_zones(symbol):
    """كشف مناطق السيولة — أين المتداولين واضعين وقف خسارة"""
    try:
...
def detect_anomaly(symbol, current_vol, current_price):
    """كشف الشذوذات — تحركات غير طبيعية"""
    anomalies = []
...
def analyze_microstructure(symbol):
    """تحليل هيكل السوق — Spread, Depth, Imbalance"""
    try:
        ...
        tight_spread = spread < avg_spread * 0.5  # Spread ضيق = سيولة عالية
...
def calc_ai_score(data):
    """درجة ذكاء اصطناعي مركّبة من كل المؤشرات"""
    score = 0.0
    ...
    if macd and macd.get("bullish"):
        ...
        if macd.get("cross"):
            score += AI_WEIGHTS["macd"] * 0.5  # بونص تقاطع
...
async def trailing_stop_loop():
    """مراقبة الـ Trailing Stop لكل الصفقات المفتوحة"""
...
def update_fear_greed():
    """تقدير Fear & Greed من بيانات السوق"""
    ...
    try:
        ...
        log.info(f"🌡️ Fear & Greed: {fg} — {label}")
        return fg
...
async def send_sector_heatmap():
    """خريطة حرارية للقطاعات كل ساعتين"""
    try:
        lines = []
...
def get_smart_cooldown(score):
    """Cooldown ذكي حسب قوة الإشارة"""
    if score >= 6:
        return COOLDOWN_SCORE_6
    elif score >= 5:
        return COOLDOWN_SCORE_5
    else:
        return COOLDOWN_SCORE_4
...
# ──────────────────────────────────────────
def is_quiet_hours():
    """هل نحن في الساعات الهادئة؟"""
    hour = datetime.utcnow().hour
    return QUIET_HOURS_START <= hour < QUIET_HOURS_END


def get_effective_min_volume():
    """حجم أدنى أعلى في الساعات الهادئة"""
    if is_quiet_hours():
        return MIN_DAILY_VOLUME * 2
...
async def health_check_loop():
    """فحص صحة البوت كل 30 دقيقة"""
    global last_health_ping
    while True:
        await asyncio.sleep(1800)
        try:
            # تأكد الاتصال بالبورصة
            exchange.fetch_time()
            last_health_ping = time.time()
                ...
                try:
                    ...
                    for trade in trades:
                        ...
                        if trade_key in whale_alerted_at:
                            continue
                        whale_alerted_at[trade_key] = now

                        if trade_value >= WHALE_STRONG_USDC:
                            level       = "strong"
                            whale_emoji = "🐋"
                            level_text  = "حوت ضخم"
                        else:
                        ...
                        # تحقق من الـ microstructure
                        micro = analyze_microstructure(symbol)
                        micro_txt = ""
                        if micro and micro["bullish_depth"]:
                            micro_txt = f"\n📊 عمق الشراء: `{micro['depth_ratio']}x` 💪"
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    resistance_map[symbol] = resistance
...
def analyze_symbol(symbol):
    try:
        ...
        ohlcv = exchange.fetch_ohlcv(symbol, "1m", limit=60)  # زدنا للـ Fib و VP
        if len(ohlcv) < 20:
            return None
...
async def send_hourly_radar(symbols):
    ...
    for symbol in symbols[:80]:
        try:
            ...
            rsi_v = calc_rsi(df["c"].tolist(), 7)  # RSI سريع
            if ratio > 2.0:
                hot.append((symbol, round(ratio, 1), rsi_v or 50))
        except:
            continue
...
async def send_market_mood():
    ...
    try:
        ...
        rsi_line  = f"📈 BTC RSI: `{btc_rsi}` | " + (
            ...
            "قوي 🟢" if 50 < btc_rsi < 70 else
            "محايد" if btc_rsi else ""
        ) + "\n"
    except:
        rsi_line = ""
    ...
    # إرسال هيتمات القطاعات مع مزاج السوق
...
async def run_scan():
    ...
    # ترتيب بالـ AI Score بدل score فقط
    ...
    log.info(f"✅ انتهى | إشارات: {len(found)} | أُرسل: {sent}")
...
async def main():
    ...
    await bot.send_message(
        chat_id=TELEGRAM_CHAT_ID,
        text=(
            "✅ *بوت التجميع الذكي — النسخة الأقوى* 🚀\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "📊 *المؤشرات التقنية:*\n"
            "  ├ RSI + Stochastic RSI ✅\n"
            "  ├ MACD + Histogram ✅\n"
...
async def main_scan_loop():
    """حلقة الفحص الرئيسي — مع Auto-Restart"""
